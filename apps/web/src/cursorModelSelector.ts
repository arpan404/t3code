import type { CursorModelMetadata, CursorModelOptions, ServerProviderModel } from "@ace/contracts";
...
function pickCursorModelForFamily(input: {
  ...
  for (const model of family.models) {
    const metadata = readCursorMetadata(model);
    let score = fallbackScore(metadata, family);

    const reasoningEffort = metadata ? normalizedReasoningEffort(metadata, family) : null;
    if (desired.reasoningEffort !== undefined) {
      if (reasoningEffort !== desired.reasoningEffort) {
        continue;
      }
      score += 12;
    }
    if (desired.fastMode !== undefined) {
      if ((metadata?.fastMode ?? false) !== desired.fastMode) {
        continue;
      }
      score += 8;
    }
    if (desired.thinking !== undefined) {
      if ((metadata?.thinking ?? false) !== desired.thinking) {
        continue;
      }
      score += 8;
    }
    if (desired.maxMode !== undefined) {
      if ((metadata?.maxMode ?? false) !== desired.maxMode) {
        continue;
      }
      score += 8;
    }
  ...
  return best?.model ?? family.models[0] ?? null;
}
